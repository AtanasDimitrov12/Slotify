...
export default function ServiceEditorDialog({ open, onClose, onSave }: Props) {
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          <TextField label="Service name" value={name} onChange={(e) => setName(e.target.value)} autoFocus />
          <TextField label="Duration (min)" type="number" value={durationMin} onChange={(e) => setDurationMin(e.target.value)} />
          <TextField label="Price (€)" type="number" value={priceEUR} onChange={(e) => setPriceEUR(e.target.value)} />
